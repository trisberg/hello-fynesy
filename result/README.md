...
```
tanzu apps workload create hello-fynesy -f config/workload.yaml \
  --local-path . \
  --type web
```
...
```
tanzu apps workload get hello-fynesy
```

You can assign the URL to a variable to make it easier to reference it:

```
URL=$(kubectl get services.serving.knative.dev/hello-fynesy -o=jsonpath='{.status.url}')
 ```
